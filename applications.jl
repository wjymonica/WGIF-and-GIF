...
function dehazing(I, method, r = 60, e = 7, ratio = 0.98, omega = 31/32, lambda = 1/1000, haze_level = 0.03125)
    c, h, w = size(I)
    I_rgb = Float64.(I);
    ...
    t_smoothed = t
    if method == "wgif"
        t_smoothed = wgif(I_gray, t, r, lambda)
    end
    if method == "gif"
        t_smoothed = gif(I_gray, t, r, lambda)
    end
